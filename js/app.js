 ...
 */
 ...
 const App = {
	...
	* ✅ VERSION 2.2.2 - Avec correctifs bugs #1, #4
	*/
	...
	* ✅ VERSION 2.2.2 - BUG FIX #5: Correction double réduction récupération
	*/
	generateLongRun(weeklyKm, numTrainingDays, phase, isRecovery, paces, weekNumber, raceDistanceKm, phaseWeekNumber) {
		...
		// ✅ BUG FIX #5: Correction double réduction
		// Si récupération, recalculer sur volume non-réduit puis réduire modérément
		let baseKm;
		if (isRecovery) {
			// Estimer le volume de charge (inverse de la réduction 70%)
			const estimatedChargeKm = weeklyKm / 0.70;
			baseKm = Math.min(estimatedChargeKm * percentage, cap);
			// Réduire modérément la sortie longue (80% au lieu de 70%)
			baseKm = baseKm * 0.80;
		} else {
			baseKm = Math.min(weeklyKm * percentage, cap);
		}
    
		let km = Math.round(baseKm);
